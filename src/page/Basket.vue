<template>
  <body>
    <ul class="flex justify-between space-x-6">
      ...
      >
        <router-link to="order"
          ><button class="text-1xl text-black font-bold text-center">
            Continuer mes achats
          </button></router-link
        >
      </li>
    </ul>
    <h3 class="text-4xl text-black font-bold text-center">Votre panier</h3>
    ...
    <div v-for="(plat, index) in panier" :key="index" :plat="plat">
      <div
        class="grid space-y-1 p-10 m-20 border-green-700 rounded-2xl bg-green-700 grid-cols-5 item-center gap-3"
      >
        <img
          :src="plat.image"
          alt=""
          class="hover:scale-125 text-center duration-300 w-128"
        />
        <span type="nom" id="nom" class="font-bold text-white"
          >{{ plat.nom }}
        </span>
        <span type="number" id="prix" class="font-extrabold text-white"
          >{{ plat.prix }}
        </span>
        <span
        ...
        <div v-for="(plat, index) in panier" :key="index" :plat="plat" class="">
          {{}}
        </div>
        <div class="">
          <select name="number" id="number" class="p-2 m-2 text-end rounded-xl">
