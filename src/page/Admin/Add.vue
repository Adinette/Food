<template>
    <div class="max-w-screen-xl mx-auto flex flex-col items-center space-y-4 p-6">
        <h2 class="uppercase text-black text-2xl font-bold">Formulaire d'ajout des categories</h2>
  ...
  event.preventDefault(); 
    ...
    if(nom.value==""|| image.value==""){
        ...
        try{
            ...
            const result = request('categorie', 'POST', { "Authorization": localStorage.getItem('token') }, toSend, false);
            result.then(data => {})
